...
#include <fstream>
#include <sstream>
#include </home/pi/project_omega-visionrevised/src/raspicam_cv.h>
...
void actionMove(int actCode)
{
	if (actCode != actHistory){
		EnableMove=0; //keluar dari pengulangan aksi sebelumnya
		while(ready==0){}
		kodegerak=actCode;
		actHistory=actCode;
		EnableMove=1; //ACTION!
...
int main ( int argc,char **argv )
 {
	...
	while (1)
		...
		if(EnableVision==1){
			...
			if(useMorphOps) {morphOps(h_threshold);}
			if(scan_bola)
			{
			//cout<<"cari_bola"<<endl;
			//scan_gawang=true;
...
*/
	...
	if(gawang)
	{
		...
		else
		{
			...
			dxl_write_word(HEADX,30,toleh_lokasi);
			dxl_write_word(HEADY,30,angguk_lokasi);
...
{
	EnableMove=1;
	actionMove(a); //class move akan otomatis mengeksekusi kode gerak
	switch (a)
	{
...
void scan_ball ()
{
	bool c;
	//cout<<"scanning"<<endl;
	dxl_write_word( HEADY, 30, 150);//32
	dxl_write_word( HEADX, 30, 150);
...
//	cout<<"statX   = "<<statX<<endl;
	dxl_write_word( HEADY, 30, serY);
	dxl_write_word( HEADX, 30, serX);
...
void trackBall2(int temp,Mat &imgThresh)//int Mat imgThresh) 
{
	...
	Mat imgTracking;
		...
		dxl_write_word( HEADY, 30, 260);
		dxl_write_word( HEADX, 30, 512);
	ii=ii+1;
	Moments moment = moments(imgThresh); //We give the binary converted frames for calculating the moments
	double area = moment.m00; //Sum of all white color pixels
